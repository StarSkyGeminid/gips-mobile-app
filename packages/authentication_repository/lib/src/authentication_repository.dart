...
class AuthenticationRepository {
  ...
  Future<void> logIn({
    ...
    await _localData.token.setToken(resultJson['accessToken'] as String);
  ...
  Future<void> register({required RegisterModel registerModel}) async {
    ...
    if (response.statusCode == 400) throw UsernameAlreadyTaken();
  ...
  Future<bool> resetPassword({required String email}) async {
    final locationRequest = Uri.http(
      _baseUrl,
      '/api/reset-password',
    );
    ...
    final response =
        await _httpClient.post(locationRequest, body: body, headers: headers);
